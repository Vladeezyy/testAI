...
test.describe('AdvancedMC Product Search - Suite 3.2', () => {
  ...
  test('TC3.2: Search for reconfigurable FPGA AMC with 100G I/O and DDR3 buffering', async ({ page }, testInfo) => {
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('Reconfigurable FPGA AMC with 100G I/O and DDR3');
    ...
    const searchPrompt = 'Do you have a reconfigurable FPGA-based AMC board that offers 100G-class I/O and large DDR3 buffering (multiple channels totaling around 4GB) for high-speed data processing?';
    const maxProducts = 5;
    const expectedCategory = 'AdvancedMC';
    const originalProductUrl = 'amc534';
    
    await description(`
**Test Objective:** Search for reconfigurable FPGA AMC boards with 100G I/O and large DDR3 buffering

**Search Query:**
${searchPrompt}

**Requirements:**
- Reconfigurable FPGA-based AMC board
- 100G-class I/O
- Large DDR3 buffering (multiple channels, ~4GB total)
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `AdvancedMC_TC3.2_${timestamp}.md`);
...
## 🎯 Search Requirements
- **Type:** Reconfigurable FPGA-based AMC board
- **I/O:** 100G-class
- **Memory:** Large DDR3 buffering (~4GB total, multiple channels)
