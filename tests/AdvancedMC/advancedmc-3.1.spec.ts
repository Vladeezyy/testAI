...
test.describe('AdvancedMC Product Search - Suite 3.1', () => {
  ...
  test('TC3.1: Search for AMC FPGA module with dual zQSFP+ 100G Ethernet', async ({ page }, testInfo) => {
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('AMC FPGA Module with Dual zQSFP+ 100G Ethernet');
    await severity(Severity.CRITICAL);
    await tag('AdvancedMC');
    await tag('BoardBot');
    await tag('FPGA');
    await tag('100G-Ethernet');
    await tag('zQSFP+');
    await owner('QA Team');
    
    const searchPrompt = 'Hi, I need an AMC FPGA module with dual zQSFP+ front-panel connectors supporting 100G Ethernet (one port at 100G and another up to ~40G). What matches that?';
    const maxProducts = 5;
    const expectedCategory = 'AdvancedMC';
    const originalProductUrl = 'amc534';
    
    await description(`
**Test Objective:** Search for AMC FPGA modules with dual zQSFP+ and 100G Ethernet support
...
**Requirements:**
- AMC FPGA module
- Dual zQSFP+ front-panel connectors
- 100G Ethernet support (one port 100G, another ~40G)
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `AdvancedMC_TC3.1_${timestamp}.md`);
...
## 🎯 Search Requirements
- **Type:** AMC FPGA module
- **Connectors:** Dual zQSFP+ front-panel
- **Networking:** 100G Ethernet (one port 100G, another ~40G)
