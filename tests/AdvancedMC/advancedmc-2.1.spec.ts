...
test.describe('AdvancedMC Product Search - Suite 2.1', () => {
  ...
  test('TC2.1: Search for MicroTCA 1U enclosure with PCIe Gen3', async ({ page }, testInfo) => {
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('MicroTCA 1U Enclosure with PCIe Gen3');
    await severity(Severity.CRITICAL);
    await tag('MicroTCA');
    await tag('BoardBot');
    await tag('PCIe-Gen3');
    await tag('1U-Enclosure');
    await owner('QA Team');
    
    const searchPrompt = 'Need a MicroTCA 1U enclosure providing PCIe Gen3 to each AMC slot (dual x4 or single x8), with adjacent-slot backplane connections and dual redundant rear hot-swap PSUs';
    const maxProducts = 5;
    const expectedCategory = 'MicroTCA';
    const originalProductUrl = 'vt951';
    
    await description(`
**Test Objective:** Search for MicroTCA 1U enclosures with PCIe Gen3 support

**Search Query:**
${searchPrompt}

**Requirements:**
- MicroTCA 1U enclosure
- PCIe Gen3 to each AMC slot (dual x4 or single x8)
- Adjacent-slot backplane connections
- Dual redundant rear hot-swap PSUs
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `MicroTCA_TC2.1_${timestamp}.md`);
...
## 🎯 Search Requirements
- **Type:** MicroTCA 1U enclosure
- **PCIe:** Gen3 to each AMC slot (dual x4 or single x8)
- **Backplane:** Adjacent-slot connections
- **PSU:** Dual redundant rear hot-swap
