...
test.describe('AdvancedMC Product Search - Suite 2.2', () => {
  ...
  test('TC2.2: Search for AMC shelf manager module', async ({ page }) => {
    test.setTimeout(120000);
    await page.setViewportSize({ width: 1920, height: 1080 });
    
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('AMC Shelf Manager Module');
    ...
    await description([
      '**Test Objective:** TC2.2: Search for AMC shelf manager module',
      '',
      '**Search Query:**',
      searchPrompt,
      '',
      '**Requirements:**',
      '- Shelf manager module (AMC manager card)\n- MicroTCA/ATCA compatibility\n- IPMI, sensor monitoring, hot-swap control',
      '',
      '**Validation:**',
    ...
    await test.step('AI Product Validation', async () => {
      await TestBase.validateWithAI(page, boardBotPage, products, searchPrompt, expectedCategory, 'AdvancedMC_TC2.2');
    });
