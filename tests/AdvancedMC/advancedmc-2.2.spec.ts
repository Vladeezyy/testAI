...
test.describe('AdvancedMC Product Search - Suite 2.2', () => {
 ...
 test('TC2.2: Search for front-to-rear cooled 1U chassis with modern shelf manager', async ({ page }, testInfo) => {
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('MicroTCA 1U Chassis with Modern Shelf Manager');
    await severity(Severity.CRITICAL);
    await tag('MicroTCA');
    await tag('BoardBot');
    await tag('Shelf-Manager');
    await tag('GPS-IEEE1588');
    await owner('Vlad');
    
   const searchPrompt = 'Do you have a front-to-rear cooled 1U chassis with 6 AMC slots and a modern shelf manager/MCH that supports things like GPS, IEEE1588, SyncE, or NTP, plus low-jitter clocking?';
    const maxProducts = 5;
