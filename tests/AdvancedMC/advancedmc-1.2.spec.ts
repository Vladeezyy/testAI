...
test.describe('AdvancedMC Product Search - Suite 1.2', () => {
  ...
  test('TC2: Search for AdvancedMC Xeon-based CPU board with ECC DDR4 and 10G Ethernet', async ({ page }, testInfo) => {
    // Allure metadata
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('AdvancedMC Xeon CPU Board with ECC DDR4');
    ...
    // Test configuration
    const searchPrompt = 'Do you have an AdvancedMC Xeon-based CPU board for high-performance workloads, with ECC DDR4 memory (about 16GB) and improved Ethernet via 10G backplane support?';
    const maxProducts = 5;
    ...
    await description(`
**Test Objective:** Search for AdvancedMC Xeon-based CPU boards with specific memory and networking requirements

**Search Query:**
${searchPrompt}

**Requirements:**
- Xeon-based CPU board
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `AdvancedMC_TC2_${timestamp}.md`);
