...
test.describe('AdvancedMC Product Search - Suite 2.3', () => {
 ...
 test('TC2.3: Search for rugged MicroTCA chassis for Mil/Aero', async ({ page }, testInfo) => {
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('Rugged MicroTCA Chassis for Mil/Aero');
   ...
   const searchPrompt = "I'm looking for a rugged 1U MicroTCA chassis for Mil/Aero, designed to handle shock and vibration, with around 6 mid-size AMC slots and redundant hot-swappable PSUs";
    const maxProducts = 5;
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `MicroTCA_TC2.3_${timestamp}.md`);
