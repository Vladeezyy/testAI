...
test.describe('MicroTCA Product Search - Suite 3.2', () => {
  ...
  test('TC3.2: Search for MicroTCA μTCA.4 AMC Data Acquisition Card capability', async ({ page }, testInfo) => {
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('MicroTCA μTCA.4 AMC Data Acquisition Card');
    ...
    const searchPrompt = "Hey, we're building a MicroTCA system and need the edge connectors for the cards. Do you have ones that handle FPGA-Buffering and Fiber-Ports links?";
    const maxProducts = 5;
    const expectedCategory = 'MicroTCA';
    const originalProductUrl = 'amc523';
    
    await description(`
**Test Objective:** Search for MicroTCA μTCA.4 AMC Data Acquisition Card and Fiber-Ports capabilities

**Search Query:**
${searchPrompt}

**Requirements:**
- μTCA.4 AMC data acquisition card
- FPGA-Buffering capability
- Fiber-Ports links support
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `MicroTCA_TC3.2_${timestamp}.md`);
...
## 🎯 Search Requirements
- **System:** MicroTCA
- **Component:** Data acquisition card
- **Features:** FPGA-Buffering capability, Fiber-Ports links
