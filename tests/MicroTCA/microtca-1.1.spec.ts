...
test.describe('MicroTCA Product Search - Suite 1.1', () => {
  ...
  test('TC1.1: Search for MicroTCA edge connectors with hot-swap capability', async ({ page }, testInfo) => {
    ...
    await description(`
**Test Objective:** Search for MicroTCA edge connectors with hot-swap and high-speed capabilities
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `MicroTCA_TC1.1_${timestamp}.md`);
...
## 🎯 Search Requirements
- **System:** MicroTCA
- **Component:** Edge connectors for cards
- **Features:** Hot-swap capability, high-speed links
