...
test.describe('MicroTCA Product Search - Suite 3.1', () => {
  ...
  test('TC3.1: Search for MicroTCA AMC Signal Generation and Capture Module capability', async ({ page }, testInfo) => {
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('MicroTCA AMC Signal Generation and Capture Module');
    ...
    const searchPrompt = "Hey, we're building a MicroTCA system and need the edge connectors for the cards. Do you have ones that handle FPGA-Processing and Front-Panel-IO links?";
    const maxProducts = 5;
    const expectedCategory = 'MicroTCA';
    const originalProductUrl = 'amc523';
    
    await description(`
**Test Objective:** Search for MicroTCA AMC Signal Generation and Capture Module and Front-Panel-IO capabilities
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `MicroTCA_TC3.1_${timestamp}.md`);
...
## 🎯 Search Requirements
- **System:** MicroTCA
- **Component:** Signal generation/capture module
- **Features:** FPGA-Processing capability, Front-Panel-IO links
