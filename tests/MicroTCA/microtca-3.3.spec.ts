...
test.describe('MicroTCA Product Search - Suite 3.3', () => {
  ...
  test('TC3.3: Search for MicroTCA MTCA.4 AMC Dual-DAC with FPGA and μRTM capability', async ({ page }, testInfo) => {
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('MicroTCA MTCA.4 AMC Dual-DAC with FPGA and μRTM');
    ...
    const searchPrompt = "Hey, we're building a MicroTCA system and need the edge connectors for the cards. Do you have ones that handle DDR3-Buffering and Quad-SFP-Plus links?";
    const maxProducts = 5;
    const expectedCategory = 'MicroTCA';
    const originalProductUrl = 'amc523';
    
    await description(`
**Test Objective:** Search for MicroTCA MTCA.4 AMC Dual-DAC with FPGA and μRTM and Quad-SFP-Plus capabilities
...
- DDR3-Buffering capability
- Quad-SFP-Plus links support
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `MicroTCA_TC3.3_${timestamp}.md`);
...
## 🎯 Search Requirements
- **System:** MicroTCA
- **Component:** Dual-DAC module with μRTM
- **Features:** DDR3-Buffering capability, Quad-SFP-Plus links
