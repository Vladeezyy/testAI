...
test.describe('MicroTCA Product Search - Suite 2.3', () => {
  ...
  test('TC2.3: Search for MicroTCA 2U MTCA.4 Chassis with µRTM Support capability', async ({ page }, testInfo) => {
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('MicroTCA 2U MTCA.4 Chassis with µRTM Support');
    ...
    const searchPrompt = "Hey, we're building a MicroTCA system and need the edge connectors for the cards. Do you have ones that handle uRTM and 80W-Per-Slot links?";
    const maxProducts = 5;
    const expectedCategory = 'MicroTCA';
    const originalProductUrl = 'rackpakm5-1-the-next-step-mtca-4-chassis';
    
    await description(`
**Test Objective:** Search for MicroTCA 2U MTCA.4 Chassis with µRTM Support and 80W-Per-Slot capabilities

**Search Query:**
${searchPrompt}

**Requirements:**
- 2U 19" MicroTCA chassis with up to 6 AMC slots
- uRTM capability
- 80W-Per-Slot links support
...
## 🎯 Search Requirements
- **System:** MicroTCA
- **Component:** 2U MTCA.4 chassis
- **Features:** uRTM capability, 80W-Per-Slot links
