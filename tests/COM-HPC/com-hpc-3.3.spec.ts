...
test.describe('COM-HPC Product Search - Suite 3.3', () => {
  
  test.beforeAll(async () => {
    await TestBase.setupReportCleanup(__dirname);
  });
  
  test('TC3.3: Search for COM-HPC Server Type D with Xeon D-2100 and 256GB RAM', async ({ page }) => {
    test.setTimeout(240000); // 4 minutes for AI validation
    await page.setViewportSize({ width: 1920, height: 1080 });
    
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('COM-HPC Server Type D - Xeon D-2100');
    await severity(Severity.CRITICAL);
    await tag('COM-HPC');
    await tag('BoardBot');
    await tag('Server-Type-D');
    await tag('Xeon-D-2100');
    await tag('16-Core');
    await tag('256GB-RAM');
    await tag('RDIMM-LRDIMM');
    await tag('PCIe-Gen3');
    await tag('10GBASE-KR');
    await owner('Vladyslav');
    
    const searchPrompt = "Looking for a COM-HPC® Server Type module, Size D (160×160 mm) based on Intel Xeon D-2100 series (up to 16 cores, ~100W TDP), supporting up to 256GB RDIMM/LRDIMM via 4 DIMM slots, with ~44 PCIe Gen3 lanes, SATA III + USB3/2.0, and multi-port 10GBASE-KR (up to 4 ports) plus 1GbE, operating 0–60°C and CE/FCC Class B certified.";
    const maxProducts = 5;
    const expectedCategories = ['COM-HPC'];
    const originalProductUrl = 'supermicro-x11sdc';
    
    await description([
      '**Test Objective:** TC3.3: Search for COM-HPC Server Type D with Xeon D-2100 and 256GB RAM',
      '',
      '**Search Query:**',
      searchPrompt,
      '',
      '**Requirements:**',
      '- COM-HPC Server Type D (160×160 mm)\n- Intel Xeon D-2100 series (up to 16 cores, ~100W TDP)\n- Up to 256GB RDIMM/LRDIMM via 4 DIMM slots\n- ~44 PCIe Gen3 lanes\n- SATA III + USB3/2.0\n- Multi-port 10GBASE-KR (up to 4 ports) + 1GbE\n- Operating: 0–60°C\n- CE/FCC Class B certified',
      '',
      '**Validation:**',
