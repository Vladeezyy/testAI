...
test.describe('COM-HPC Product Search - Suite 2.2', () => {
  
  test.beforeAll(async () => {
    await TestBase.setupReportCleanup(__dirname);
  });
  
  test('TC2.2: Search for rackmount dev server for robotics/autonomous vehicles with SOAFEE', async ({ page }) => {
    test.setTimeout(240000); // 4 minutes for AI validation
    await page.setViewportSize({ width: 1920, height: 1080 });
    
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('Rackmount Dev Server - Robotics/Autonomous');
    await severity(Severity.CRITICAL);
    await tag('COM-HPC');
    await tag('BoardBot');
    await tag('Rackmount');
    await tag('Robotics');
    await tag('Autonomous-Vehicle');
    await tag('SOAFEE');
    await tag('Autoware');
    await tag('Multi-Core');
    await owner('QA Team');
    
    const searchPrompt = "We're building an embedded/robotics or autonomous vehicle stack and want a tough rackmount dev server with tons of cores and fast networking — ideally something aligned with SOAFEE or Autoware. Anything like that?";
    const maxProducts = 5;
    const expectedCategories = ['COM-HPC'];
    const originalProductUrl = 'ampere-altra-developer-rugged';
    
    await description([
      '**Test Objective:** TC2.2: Search for rackmount dev server for robotics/autonomous vehicles with SOAFEE',
      '',
      '**Search Query:**',
      searchPrompt,
      '',
      '**Requirements:**',
      '- Tough rackmount dev server\n- Embedded/robotics/autonomous vehicle stack\n- High core count\n- Fast networking\n- SOAFEE or Autoware aligned',
      '',
      '**Validation:**',
    ...
    await test.step('AI Product Validation', async () => {
      await TestBase.validateWithAI(page, boardBotPage, products, searchPrompt, expectedCategories[0], 'COM-HPC_TC2.2');
    });
