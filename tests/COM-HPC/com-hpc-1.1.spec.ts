...
test.describe('COM-HPC Product Search - Suite 1.1', () => {
  
  test.beforeAll(async () => {
    await TestBase.setupReportCleanup(__dirname);
  });
  
  test('TC1.1: Search for compact COM-HPC module with Intel Core Ultra and AI/GPU', async ({ page }) => {
    test.setTimeout(240000); // 4 minutes for AI validation
    await page.setViewportSize({ width: 1920, height: 1080 });
    
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('Compact COM-HPC with Intel Core Ultra and AI');
    ...
    await description([
      '**Test Objective:** TC1.1: Search for compact COM-HPC module with Intel Core Ultra and AI/GPU',
      '',
      '**Search Query:**',
      searchPrompt,
      '',
      '**Requirements:**',
      '- Compact compute module\n- Intel Core Ultra chips\n- Built-in AI/GPU power\n- No extra accelerators needed',
      '',
      '**Validation:**',
    ...
    await test.step('AI Product Validation', async () => {
      await TestBase.validateWithAI(page, boardBotPage, products, searchPrompt, expectedCategories[0], 'COM-HPC_TC1.1');
    });
