...
test.describe('COM Express Product Search - Suite 1.1', () => {
  
  test.beforeAll(async () => {
    await TestBase.setupReportCleanup(__dirname);
  });
  
  test('TC1.1: Search for COM Express module with Intel processors for industrial systems', async ({ page }) => {
    test.setTimeout(240000); // 4 minutes for AI validation
    await page.setViewportSize({ width: 1920, height: 1080 });
    
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('COM Express with Intel - Industrial/Embedded');
    ...
    const searchPrompt = "Hi, I'm looking for a COM Express compute module with newer Intel processors — something efficient but still powerful, with good I/O and networking for an industrial/embedded system. What do you recommend?";
    const maxProducts = 5;
    const expectedCategories = ['COM EXPRESS', 'COM Express'];
    const originalProductUrl = 'msc-c6b-tlh-2';
    
    await description([
      '**Test Objective:** TC1.1: Search for COM Express module with Intel processors for industrial systems',
      '',
      '**Search Query:**',
      searchPrompt,
      '',
      '**Requirements:**',
      '- COM Express compute module\n- Newer Intel processors\n- Efficient and powerful\n- Good I/O and networking\n- Industrial/embedded system',
      '',
      '**Validation:**',
    ...
    await test.step('AI Product Validation', async () => {
      await TestBase.validateWithAI(page, boardBotPage, products, searchPrompt, expectedCategories[0], 'COMExpress_TC1.1');
    });
