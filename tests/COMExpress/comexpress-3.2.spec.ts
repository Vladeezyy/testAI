...
test.describe('COM Express Product Search - Suite 3.2', () => {
  
  test.beforeAll(async () => {
    await TestBase.setupReportCleanup(__dirname);
  });
  
  test('TC3.2: Search for COM Express with DDR3 and GbE for long-lived legacy design', async ({ page }) => {
    test.setTimeout(240000); // 4 minutes for AI validation
    await page.setViewportSize({ width: 1920, height: 1080 });
    
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('COM Express Legacy Design - DDR3 GbE');
    ...
    await tag('DDR3');
    await tag('Gigabit-Ethernet');
    await tag('Expansion');
    await owner('Vladyslav');
    
    const searchPrompt = "We need a COM Express board for a legacy/long-lived design — Intel-based, DDR3 memory, gigabit Ethernet, and good expansion options. Any suggestions?";
    const maxProducts = 5;
    const expectedCategories = ['COM EXPRESS', 'COM Express'];
    const originalProductUrl = 'com-45sp';
    
    await description([
      '**Test Objective:** TC3.2: Search for COM Express with DDR3 and GbE for long-lived legacy design',
      '',
      '**Search Query:**',
      searchPrompt,
      '',
      '**Requirements:**',
      '- COM Express board\n- Legacy/long-lived design\n- Intel-based\n- DDR3 memory\n- Gigabit Ethernet\n- Good expansion options',
      '',
      '**Validation:**',
