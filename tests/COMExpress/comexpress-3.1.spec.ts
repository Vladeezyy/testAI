...
test.describe('COM Express Product Search - Suite 3.1', () => {
  
  test.beforeAll(async () => {
    await TestBase.setupReportCleanup(__dirname);
  });
  
  test('TC3.1: Search for COM Express with Intel Core 2 Duo for legacy embedded systems', async ({ page }) => {
    test.setTimeout(240000); // 4 minutes for AI validation
    await page.setViewportSize({ width: 1920, height: 1080 });
    
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('COM Express Legacy - Core 2 Duo');
    await severity(Severity.CRITICAL);
    await tag('COM-Express');
    await tag('BoardBot');
    await tag('Intel-Core-2-Duo');
    await tag('Legacy');
    await tag('Networking');
    await tag('USB');
    await owner('Vladyslav');
    
    const searchPrompt = "Hi, I'm looking for a COM Express module for an embedded system with an older Intel Core 2 Duo–class processor, solid networking, and plenty of USB. What modules fit that?";
    const maxProducts = 5;
    const expectedCategories = ['COM EXPRESS', 'COM Express'];
    const originalProductUrl = 'com-45sp';
    
    await description([
      '**Test Objective:** TC3.1: Search for COM Express with Intel Core 2 Duo for legacy embedded systems',
      '',
      '**Search Query:**',
      searchPrompt,
      '',
      '**Requirements:**',
      '- COM Express module\n- Intel Core 2 Duo-class processor\n- Solid networking\n- Multiple USB ports\n- Embedded system',
      '',
      '**Validation:**',
    ...
    await test.step('AI Product Validation', async () => {
      await TestBase.validateWithAI(page, boardBotPage, products, searchPrompt, expectedCategories[0], 'COMExpress_TC3.1');
    });
