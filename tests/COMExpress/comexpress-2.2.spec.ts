...
test.describe('COM Express Product Search - Suite 2.2', () => {
  
  test.beforeAll(async () => {
    await TestBase.setupReportCleanup(__dirname);
  });
  
  test('TC2.2: Search for COM Express with DDR4, USB, and 4K multi-monitor support', async ({ page }) => {
    test.setTimeout(120000);
    await page.setViewportSize({ width: 1920, height: 1080 });
    
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('COM Express Embedded PC - 4K Multi-Monitor');
    ...
    await tag('USB');
    await tag('4K');
    await tag('Multi-Monitor');
    await tag('Embedded-PC');
    await owner('Vladyslav');
    
    const searchPrompt = "We're building a small embedded PC that needs fast DDR4 memory, lots of USB, and support for 2–3 monitors (preferably 4K). Do you have a COM Express module for that?";
    const maxProducts = 5;
    const expectedCategories = ['COM EXPRESS', 'COM Express'];
    const originalProductUrl = 'msc-c6c-slu-2';
    
    await description([
      '**Test Objective:** TC2.2: Search for COM Express with DDR4, USB, and 4K multi-monitor support',
      '',
      '**Search Query:**',
      searchPrompt,
      '',
      '**Requirements:**',
      '- Small embedded PC\n- Fast DDR4 memory\n- Multiple USB ports\n- 2–3 monitor support (4K)',
      '',
      '**Validation:**',
    ...
    await test.step('AI Product Validation', async () => {
      await TestBase.validateWithAI(page, boardBotPage, products, searchPrompt, expectedCategories[0], 'COMExpress_TC2.2');
    });
