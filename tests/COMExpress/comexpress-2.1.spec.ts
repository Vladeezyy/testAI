...
test.describe('COM Express Product Search - Suite 2.1', () => {
  
  test.beforeAll(async () => {
    await TestBase.setupReportCleanup(__dirname);
  });
  
  test('TC2.1: Search for compact COM Express with Intel Core and graphics for multi-display', async ({ page }) => {
    test.setTimeout(120000);
    await page.setViewportSize({ width: 1920, height: 1080 });
    
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('Compact COM Express - Multi-Display Graphics');
    await severity(Severity.CRITICAL);
    await tag('COM-Express');
    await tag('BoardBot');
    await tag('Intel-Core');
    await tag('Graphics');
    await tag('Multi-Display');
    await tag('High-Res');
    await owner('Vladyslav');
    
    const searchPrompt = "Hi, I need a really compact COM Express module with an Intel Core processor and good built-in graphics, because we want to drive multiple high-res displays. Anything like that?";
    const maxProducts = 5;
    const expectedCategories = ['COM EXPRESS', 'COM Express'];
    const originalProductUrl = 'msc-c6c-slu-2';
    
    await description([
      '**Test Objective:** TC2.1: Search for compact COM Express with Intel Core and graphics for multi-display',
      '',
      '**Search Query:**',
      searchPrompt,
      '',
      '**Requirements:**',
      '- Compact COM Express module\n- Intel Core processor\n- Good built-in graphics\n- Multiple high-res display support',
      '',
      '**Validation:**',
    ...
    await test.step('AI Product Validation', async () => {
      await TestBase.validateWithAI(page, boardBotPage, products, searchPrompt, expectedCategories[0], 'COMExpress_TC2.1');
    });
