...
test.describe('CompactPCI Product Search - Suite 2.1', () => {
  
  test.beforeAll(async () => {
    await TestBase.setupReportCleanup(__dirname);
  });
  
  test('TC2.1: Search for 3U CompactPCI Serial NVMe M.2 carrier board', async ({ page }) => {
    test.setTimeout(240000); // 4 minutes for AI validation
    await page.setViewportSize({ width: 1920, height: 1080 });
    
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('3U CompactPCI Serial NVMe M.2 Carrier');
    ...
    await description([
      '**Test Objective:** TC2.1: Search for 3U CompactPCI Serial NVMe M.2 carrier board',
      '',
      '**Search Query:**',
      searchPrompt,
      '',
      '**Requirements:**',
      '- 3U CompactPCI Serial card\n- NVMe M.2 SSD support\n- Carrier board for multiple SSDs',
      '',
      '**Validation:**',
    ...
    await test.step('AI Product Validation', async () => {
      await TestBase.validateWithAI(page, boardBotPage, products, searchPrompt, expectedCategories[0], 'CompactPCI_TC2.1');
    });
