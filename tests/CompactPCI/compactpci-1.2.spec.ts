...
test.describe('CompactPCI Product Search - Suite 1.2', () => {
  
  test.beforeAll(async () => {
    await TestBase.setupReportCleanup(__dirname);
  });
  
  test('TC1.2: Search for 4U CompactPCI Serial backplane with high-speed interfaces', async ({ page }) => {
    test.setTimeout(240000); // 4 minutes for AI validation
    await page.setViewportSize({ width: 1920, height: 1080 });
    
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('4U CompactPCI Serial High-Speed Backplane');
    ...
    await description([
      '**Test Objective:** TC1.2: Search for 4U CompactPCI Serial backplane with high-speed interfaces',
      '',
      '**Search Query:**',
      searchPrompt,
      '',
      '**Requirements:**',
      '- 4U CompactPCI Serial rack\n- 9 slots\n- High-speed backplane (PCIe/USB/SATA/Ethernet)',
      '',
      '**Validation:**',
    ...
    await test.step('AI Product Validation', async () => {
      await TestBase.validateWithAI(page, boardBotPage, products, searchPrompt, expectedCategories[0], 'CompactPCI_TC1.2');
    });
