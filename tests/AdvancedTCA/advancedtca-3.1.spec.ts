...
test.describe('AdvancedTCA Product Search - Suite 3.1', () => {
  ...
  test('TC3.1: Search for ATCA rear module with Ethernet and PCIe', async ({ page }, testInfo) => {
    ...
    await description(`
**Test Objective:** Search for ATCA rear modules with I/O expansion capabilities
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `AdvancedTCA_TC3.1_${timestamp}.md`);
...
## 🎯 Search Requirements
- **Type:** ATCA rear module
- **I/O:** Ethernet ports expansion
- **Connectivity:** 10G links
- **Interface:** PCIe connection to the back
