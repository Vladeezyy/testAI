...
test.describe('AdvancedTCA Product Search - Suite 2.2', () => {
  ...
  test('TC2.2: Search for high-end telecom ATCA platform with high bandwidth', async ({ page }, testInfo) => {
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('High-End Telecom ATCA Platform');
    ...
    const searchPrompt = 'We are building a high-end telecom/data-com platform and need something reliable that supports very high bandwidth and lots of processing cores. Ideally an ATCA solution made for 24/7 network traffic. Any good options?';
    const maxProducts = 5;
    ...
    await description(`
**Test Objective:** Search for high-end telecom/data-com ATCA platforms

**Search Query:**
${searchPrompt}

**Requirements:**
- High-end telecom/data-com platform
- Reliable for 24/7 operation
- Very high bandwidth support
- Lots of processing cores
- ATCA solution for network traffic
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `AdvancedTCA_TC2.2_${timestamp}.md`);
...
## 🎯 Search Requirements
- **Platform:** High-end telecom/data-com
- **Reliability:** 24/7 operation
- **Bandwidth:** Very high support
- **Processing:** Lots of cores
- **Solution Type:** ATCA for network traffic
