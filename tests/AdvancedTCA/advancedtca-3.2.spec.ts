...
test.describe('AdvancedTCA Product Search - Suite 3.2', () => {
  ...
  test('TC3.2: Search for ATCA RTM with expanded network connectivity', async ({ page }, testInfo) => {
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('ATCA RTM with Network and Management Access');
    ...
    await description(`
**Test Objective:** Search for ATCA rear transition modules with expanded connectivity

**Search Query:**
${searchPrompt}

**Requirements:**
- Expand rear connectivity for ATCA
- More network ports (fiber + copper)
- Management access in the back
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `AdvancedTCA_TC3.2_${timestamp}.md`);
...
## 🎯 Search Requirements
- **Purpose:** Expand ATCA rear connectivity
- **Network Ports:** Fiber + copper
- **Management:** Rear management access
- **Type:** Rear transition modules
