...
test.describe('AdvancedTCA Product Search - Suite 3.2', () => {
  ...
  test('TC3.2: Search for ATCA RTM with expanded network connectivity', async ({ page }) => {
    ...
    await description([
      '**Test Objective:** TC3.2: Search for ATCA RTM with expanded network connectivity',
      '',
      '**Search Query:**',
      searchPrompt,
      '',
      '**Requirements:**',
      '- Expand ATCA rear connectivity\n- More network ports (fiber + copper)\n- Management access in the back\n- Rear transition module compatibility',
      '',
      '**Validation:**',
    ...
    await test.step('AI Product Validation', async () => {
      await TestBase.validateWithAI(page, boardBotPage, products, searchPrompt, expectedCategory, 'AdvancedTCA_TC3.2');
    });
