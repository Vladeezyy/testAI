...
test.describe('AdvancedTCA Product Search - Suite 2.3', () => {
  ...
  test('TC2.3: Search for Netarium-14 ATCA with 40GbE and 5-NINES availability', async ({ page }, testInfo) => {
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('Netarium-14 ATCA Reference System');
    ...
    const searchPrompt = 'Looking for Advantech Netarium ATCA reference systems, especially the Netarium-14, with 40GbE backplane/switching, carrier-grade chassis (cooling, power, shelf management) and 5-NINES availability, optimized for high-density packet processing use cases like policy/charging, network security, real-time traffic monitoring, load balancing, and analytics.';
    const maxProducts = 5;
    ...
    await description(`
**Test Objective:** Search for Advantech Netarium-14 ATCA reference systems

**Search Query:**
${searchPrompt}

**Requirements:**
- Advantech Netarium ATCA reference systems
- Netarium-14 specific model
- 40GbE backplane/switching
- Carrier-grade chassis (cooling, power, shelf management)
- 5-NINES availability
- High-density packet processing
- Use cases: policy/charging, network security, traffic monitoring, load balancing, analytics
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `AdvancedTCA_TC2.3_${timestamp}.md`);
...
## 🎯 Search Requirements
- **Product:** Advantech Netarium-14 ATCA reference system
- **Backplane:** 40GbE backplane/switching
- **Chassis:** Carrier-grade (cooling, power, shelf management)
- **Availability:** 5-NINES
- **Processing:** High-density packet processing
- **Use Cases:** Policy/charging, security, monitoring, load balancing, analytics
...
${report.originalProduct >= 0 ? '\n⭐ = Original product that inspired this search' : ''}
        ...
        if (report.totalResults > 0) {
          ...
          console.log(`✅ Suitable (AdvancedTCA): ${report.suitableProducts}`);
