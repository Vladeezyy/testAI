...
test.describe('AdvancedTCA Product Search - Suite 1.2', () => {
  ...
  test('TC1.2: Search for short 3U ATCA chassis with strong cooling and redundant power', async ({ page }, testInfo) => {
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('Short 3U ATCA Chassis with Strong Cooling');
    await severity(Severity.CRITICAL);
    await tag('AdvancedTCA');
    await tag('BoardBot');
    await tag('2-Slot');
    await tag('Redundant-Power');
    await tag('Cooling');
    await owner('QA Team');
    
    const searchPrompt = 'I need a short 3U rack chassis for an ATCA setup — only two slots — but with strong cooling and redundant power since it will run non-stop. Anything like that?';
    const maxProducts = 5;
    const expectedCategory = 'AdvancedTCA';
    const originalProductUrl = 'atca-co2-ac-dc-2-slot-3u';
    
    await description(`
**Test Objective:** Search for short 3U ATCA chassis with strong cooling and redundant power

**Search Query:**
${searchPrompt}

**Requirements:**
- Short 3U rack chassis
- ATCA setup with 2 slots
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `AdvancedTCA_TC1.2_${timestamp}.md`);
...
## 🎯 Search Requirements
- **Form Factor:** Short 3U rack chassis
- **Setup:** ATCA with 2 slots
- **Cooling:** Strong cooling capability
- **Power:** Redundant for continuous operation
