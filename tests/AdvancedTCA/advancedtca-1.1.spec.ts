...
test.describe('AdvancedTCA Product Search - Suite 1.1', () => {
  ...
  test('TC1.1: Search for compact 3U ATCA chassis with 2 slots and RTM support', async ({ page }, testInfo) => {
    ...
    await description(`
**Test Objective:** Search for compact 3U AdvancedTCA chassis with PICMG 3.0/NEBS compliance

**Search Query:**
${searchPrompt}

**Requirements:**
- 3U 19-inch form factor
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `AdvancedTCA_TC1.1_${timestamp}.md`);
...
## 🎯 Search Requirements
- **Form Factor:** 3U 19-inch
- **Slots:** 2 ATCA with RTM support
- **Compliance:** PICMG 3.0 / NEBS
