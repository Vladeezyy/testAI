...
test.describe('AdvancedTCA Product Search - Suite 3.3', () => {
  ...
  test('TC3.3: Search for ATCA ARTM with PCIe x8, GbE, and 10GbE ports', async ({ page }, testInfo) => {
    ...
    await description(`
**Test Objective:** Search for ATCA I/O expansion ARTM with comprehensive connectivity

**Search Query:**
${searchPrompt}

**Requirements:**
- ATCA I/O expansion Rear Transition Module (ARTM)
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `AdvancedTCA_TC3.3_${timestamp}.md`);
...
## 🎯 Search Requirements
- **Type:** ATCA I/O expansion ARTM
- **Compatibility:** Standard ATCA carriers
- **PCIe:** x8 rear access (iPASS)
- **GbE Ports:** Multiple (SFP + RJ-45)
- **10GbE:** Dual via Zone 3 with rear SFP+
- **Management:** RS-232 and RTM IPMI
