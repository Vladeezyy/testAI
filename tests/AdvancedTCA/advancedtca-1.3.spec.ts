...
test.describe('AdvancedTCA Product Search - Suite 1.3', () => {
  ...
  test('TC1.3: Search for 2-slot ATCA chassis with 40Gbps backplane and mesh topology', async ({ page }, testInfo) => {
    await epic('Product Search');
    await feature('BoardBot Search');
    await story('2-Slot ATCA Chassis with 40Gbps Backplane');
    ...
    const searchPrompt = 'Looking for a 19" 3U 2-slot AdvancedTCA chassis (PICMG 3.0 / NEBS-ready), supporting 2 ATCA front boards + RTMs, redundant AC PSUs (~2.2kW total, N+1), push-pull hot-swap fan trays, and a 40Gbps backplane with Dual Star / Full Mesh topology options.';
    const maxProducts = 5;
    const expectedCategory = 'AdvancedTCA';
    const originalProductUrl = 'atca-co2-ac-dc-2-slot-3u';
    
    await description(`
**Test Objective:** Search for 2-slot AdvancedTCA chassis with advanced backplane topology
...
**Requirements:**
- 19" 3U form factor
- 2-slot ATCA chassis (PICMG 3.0 / NEBS-ready)
- 2 ATCA front boards + RTMs support
- Redundant AC PSUs (~2.2kW total, N+1)
- Push-pull hot-swap fan trays
- 40Gbps backplane with Dual Star / Full Mesh topology
...
- Generate detailed report with warnings for non-matching categories
    ...
    await test.step('Validate categories and generate report', async () => {
      await allureStep('Validating product categories and generating report', async () => {
        ...
        const reportPath = path.join(reportsDir, `AdvancedTCA_TC1.3_${timestamp}.md`);
...
## 🎯 Search Requirements
- **Form Factor:** 19" 3U
- **Slots:** 2 ATCA front boards + RTMs
- **Compliance:** PICMG 3.0 / NEBS-ready
- **Power:** Redundant AC PSUs (~2.2kW total, N+1)
- **Cooling:** Push-pull hot-swap fan trays
- **Backplane:** 40Gbps with Dual Star / Full Mesh topology
